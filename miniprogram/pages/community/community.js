...
Page({
  ...
  sortingbyUpdateDate(dataSet) {
    ...
    for (let i = 0; i < dataSet.length; i++) {
      let update_date_parts = dataSet.update_date[i].split('/');
      let year = parseInt(update_date_parts[0]);
      let month = parseInt(update_date_parts[1]) - 1;
      let day = parseInt(update_date_parts[2]);
      let date_object = new Date(year, month, day);
      let milliseconds = date_object.getTime();
      dataSet[i].update_date.ms = milliseconds;
    }
    dataSet.sort((a, b) => b.ms - a.ms);
  },
  sortignbyTreats(dataSet) {
    if (dataSet == undefined) return;
    dataSet.sort((a, b) => a.treat - b.treat);
  },
  ...
  onSortingChange(event) {
    ...
    else if (event.detail == 'comment') this.sortignbyTreats();
