...
Page({
  ...
  data: {
    ...
    showOutInternalItems: [],
    showOutInternshipItems: [],
    areaList: {
      province_list: {
        110000: "北京市",
        120000: "天津市",
  ...
  getHeights() {
    ...
    query
      ...
      .boundingClientRect(function (res) {
        console.log(res.top);
        let offsetTop = res.top - that.data.navBarHeight;
        that.setData({
          offsetTop: offsetTop,
        });
        console.log(that.data.offsetTop);
      })
      .exec();
  },
  ...
  search(e) {
    console.log("e", e);
    let val = (e && e.detail.value) || this.data.searchKey;
    let actionName = "getRecruitmentData";
  ...
  dataProcessing(original) {
    ...
    if (this.data.tabIndex === 0) {
      ...
      this.setData({
        recruitment_items: original,
        showOutInternalItems: original.slice(0, 20),
      });
      wx.setStorageSync("showOutInternalItems", this.data.showOutInternalItems);
    } else {
      this.setData({
        recruitment_items: original,
        showOutInternshipItems: original.slice(0, 20),
      });
      wx.setStorageSync(
        "showOutInternshipItems",
        this.data.showOutInternshipItems
      );
    }
  ...
  sortingbyUpdateDate(dataSet) {
    ...
    if (this.data.tabIndex === 0) {
      ...
      this.setData({
        recruitment_items: dataSet,
        showOutInternshipItems: dataSet.slice(0, 20),
      });
    } else {
      this.setData({
        recruitment_items: dataSet,
  ...
  sortignbyViews(dataSet) {
    if (dataSet == undefined) return;
    for (let i = 0; i < dataSet.length; i++) {
      dataSet[i].viewData == undefined
        ? (dataSet[i].viewData = 0)
        : (dataSet[i].viewData = dataSet[i].viewData);
    }
    dataSet.sort(function (a, b) {
    ...
    if (this.data.tabIndex === 0) {
      ...
      this.setData({
        recruitment_items: dataSet,
        showOutInternalItems: dataSet.slice(0, 20),
      });
    } else {
      this.setData({
        recruitment_items: dataSet,
  ...
  onSortingChange() {
    if (this.data.tabIndex3 === 0)
      this.sortingbyUpdateDate(this.data.recruitment_items);
    else if (this.data.tabIndex3 === 1)
      this.sortignbyViews(this.data.recruitment_items);
    // else if (event.detail == "treat")
    //   this.sortingbyUpdateSalary(this.data.recruitment_items);
  },
  goRecruitmentDetail(event) {
    ...
    wx.navigateTo({
      url:
        "/pages/recruitment_details/recruitment_details?_id=" +
        event.currentTarget.dataset._id,
    });
  },
  ...
  onConfirmCitySelect1(event) {
    ...
    if (event.detail.values[1] == undefined) {
      ...
      this.setData({
        isShowCitySelect1: false,
        selectedCity1:
          event.detail.values[0].name == event.detail.values[1].name
            ? event.detail.values[0].name
            : event.detail.values[0].name + " " + event.detail.values[1].name,
      });
  ...
  onClickFlitingBtn(event) {
    ...
    this.setData({
      selectedCity: [
        sc1 == "" || undefined
          ? ""
          : sc1.split(" ")[1] == undefined
          ? sc1.split(" ")[0].replace(/市$/, "")
          : sc1.split(" ")[1].replace(/市$/, ""),
        sc2 == "" || undefined
          ? ""
          : sc2.split(" ")[1] == undefined
          ? sc2.split(" ")[0].replace(/市$/, "")
          : sc2.split(" ")[1].replace(/市$/, ""),
      ],
    });
  ...
  onReachScrollViewBottom() {
    ...
    let lzyindex =
      this.data.lazyindex + 20 > items.length
        ? items.length
        : this.data.lazyindex + 20;
    ...
    if (this.data.tabIndex === 0) {
      ...
      this.setData({
        lazyindex: lzyindex,
        showOutInternalItems: items.slice(0, lzyindex),
      });
    } else {
      this.setData({
        lazyindex: lzyindex,
        showOutInternshipItems: items.slice(0, lzyindex),
      });
    }
  },
